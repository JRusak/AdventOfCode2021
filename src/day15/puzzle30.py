...
class NodeCreator:
    def __init__(self, data, dims=1):
        self.data = data
        self.dims = dims
        self.nodes, self.width = self.get_nodes()
    ...
    def create_nodes(self, table):
        nodes = []
        for dv in range(self.dims):
            for i in range(len(table)):
                sub_nodes = []
                for dh in range(self.dims):
                    for j in range(len(table[i])):
                        cost = int(table[i][j]) + dv + dh
                        if cost > 9:
                            d = cost // 10
                            cost %= 10
                            cost += d
                        new_node = Node((i, j), cost)
...
def path_counter(data):
    nodes, width = process_data(data)
    pf = PathFinder(nodes)
    pf.a_star()
    cost = pf.get_path_cost()
    # print(pf.print_nodes(width))
    return cost
